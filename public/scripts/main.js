...
(function ($) {
    ...
    function documentReadyInit() {
        ...
        $('.contact-form').on('submit', function (e) {
            ...
            $form.find('.response').html('Submitting...');
            // Prepare query string and send AJAX request
            jQuery.post({
                url: 'contact',
                data: 'email=' + escape($form.find('#email333').val())
                    + '&message=' + escape($form.find('#message335553').val())
                    + '&name=' + escape($form.find('#name335x5553').val())
                    + '&phone=' + escape($form.find('#name3355553').val())
                    + '&subject=' + escape($form.find('#name3355d553').val())

                ,
                // dataType: "json",
                // contentType: "application/json; charset=utf-8",
                success: function (msg) {
                    $form.find('.response').html("Submitted");
                    $form.find('#email333').val("")
                    $form.find('#message335553').val("")
                    $form.find('#name335x5553').val("")
                    $form.find('#name3355553').val("")
                    $form.find('#name3355d553').val("")
                },
                error: function (msg) {
                    $form.find('.response').html(msg?.responseJSON?.message);
                }
            });
        });
        ...
        $('.signup').on('submit', function (e) {
            ...
            jQuery.post({
                ...
                success: function (msg) {
                    $form.find('.response').html("Subscribed");
                },
                error: function (msg) {
                    $form.find('.response').html(msg?.responseJSON?.message);
                }
        ...
        $('.signup_landing').on('submit', function (e) {
            e.preventDefault();
            var $form = $(this);
            // update user interface
            $form.find('.response').html('Adding email address...');
            ...
            jQuery.post({
                url: 'newsletter',
                data: 'email=' + escape($form.find('.email_landing').val()),
                success: function (msg) {
                    $form.find('.response').html("Subscribed");
                },
                error: function (msg) {
